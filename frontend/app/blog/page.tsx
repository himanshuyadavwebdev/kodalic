    ...
    <main className="min-h-screen w-full bg-white font-[Inter]">
      <div className="mx-auto max-w-6xl px-6 sm:px-10 lg:px-20 pt-10 pb-16">
        ...
        <div className="mt-8 text-center">
          ...
          <h1 className="mt-4 text-3xl sm:text-5xl font-bold tracking-tight text-[#161221]">Blog</h1>
          ...
          {DEMO_BLOG_POSTS.map((post) => {
            ...
            return (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="group flex flex-col overflow-hidden rounded-[20px] border bg-white p-4 transition-transform duration-200 hover:-translate-y-1 focus-visible:outline-offset-2"
              ...
              >
                ...
                <div className="mt-4 flex flex-1 flex-col">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="rounded-full bg-black/5 px-2 py-1 text-xs text-black/60">{post.category}</span>
                    <span className="text-xs text-black/40">{post.readingTime}</span>
                  </div>
                  <h2 className="text-lg font-bold leading-tight text-[#161221] group-hover:text-black">{post.title}</h2>
                  <p className="mt-2 flex-1 text-sm leading-relaxed text-black/60 line-clamp-3">{post.description}</p>
