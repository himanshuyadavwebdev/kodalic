...
export default function TechMarquee({ isDark = false }: { isDark?: boolean }) {
  ...
  return (
    <section
      className="relative w-full overflow-hidden border-y border-black/[0.04] dark:border-white/[0.06] py-6"
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-y-0 left-0 w-24 bg-gradient-to-r from-white to-transparent dark:from-[#0a0f1e] z-10" aria-hidden />
      <div className="pointer-events-none absolute inset-y-0 right-0 w-24 bg-gradient-to-l from-white to-transparent dark:from-[#0a0f1e] z-10" aria-hidden />
      <div className="relative z-0 flex flex-col gap-4">
        <div className="relative flex overflow-hidden">
          <div className="flex items-center gap-10 whitespace-nowrap will-change-transform" style={{ animation: "marquee 32s linear infinite" }}>
            {items.map((tech, i) => (
                <div key={`${tech.name}-${i}`} className="flex items-center gap-2 px-4 py-2 rounded-full border border-black/[0.06] bg-white text-sm font-medium text-black/60 dark:border-white/10 dark:bg-white/5 dark:text-white/70">
        ...
        <div className="relative flex overflow-hidden">
          <div className="flex items-center gap-10 whitespace-nowrap will-change-transform" style={{ animation: "marquee 40s linear infinite reverse" }}>
            {items.map((tech, i) => (
              <div key={`${tech.name}-r-${i}`} className="flex items-center gap-2 px-4 py-2 rounded-full border border-black/[0.06] bg-white text-sm font-medium text-black/60 dark:border-white/10 dark:bg-white/5 dark:text-white/70">
